...
## 18. No Locks to Audit, Because There Are No Locks
**Decision**: A full-repo audit for concurrency primitives (`std::mutex`, `std::thread`, `std::atomic`, `std::condition_variable`, any locking construct at all) found zero matches in `src/` or `tests/`, confirmed by grep, not by reading the Known Limitations section and assuming it was still accurate.
**Rationale**: "Check for deadlocks" only means something where locks exist to order incorrectly. This engine is single-threaded by design (entry 1's baseline decision, never revisited), so the actual, verifiable claim is stronger than "we checked and found no deadlocks" — it's "the class of bug cannot exist here," since nothing in the codebase acquires more than zero locks. Worth stating as a checked fact rather than an inferred one: the Known Limitations section already said "single-threaded," but saying so and grepping the entire source tree to confirm nothing contradicts it are different levels of confidence, and only one of them is what actually got done here.

## 19. Expand clang-tidy, Run It For Real, Fix What It Found
**Decision**: Installed LLVM locally (`brew install llvm`) to run `clang-tidy` for the first time outside CI, expanded `.clang-tidy`'s checks to include `misc-*` and `clang-diagnostic-*` (neither enabled before, and both are where unused-code/unused-include findings actually live), and fixed every `misc-include-cleaner` finding — 44 headers relying on transitive includes instead of a direct one, one genuinely unused `#include <iostream>`.
**Rationale**: A lint config that's never actually run isn't a lint config, it's a hope. Running it surfaced a real, if narrow, category of fixable issues (include hygiene) alongside a large category of legitimate-but-non-urgent style suggestions (const-correctness, bounds-checking preferences) — see ADR-0024 for why the second category got triaged and deferred rather than blitzed through in the same pass. Zero genuine dead code or unused variables turned up anywhere, which is itself worth stating as a checked result, not an assumption.
