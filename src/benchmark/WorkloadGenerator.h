...
enum class ActionType { Insert, Cancel };
...
public:
    ...
    std::vector<BenchmarkAction> generateRandom(size_t numActions) {
        ...
        for (size_t i = 0; i < numActions; ++i) {
            ...
            actions.push_back(
                {ActionType::Insert, Order(orderIdCounter_++, priceDist(rng_), qtyDist(rng_), side, OrderType::Limit)});
        }
        return actions;
    }
    ...
    std::vector<BenchmarkAction> generateWorstCase(size_t numActions) {
        ...
        for (size_t i = 0; i < numActions; ++i) {
            ...
            actions.push_back(
                {ActionType::Insert, Order(orderIdCounter_++, fixedPrice, qtyDist(rng_), side, OrderType::Limit)});
        }
        return actions;
    }
    ...
    std::vector<BenchmarkAction> generateHeavyCancels(size_t numActions) {
        ...
        for (size_t i = 0; i < numActions; ++i) {
            ...
            if (actionDist(rng_) < 0.7 || activeOrders.empty()) {
                ...
                Side side = sideDist(rng_) == 0 ? Side::Buy : Side::Sell;
                actions.push_back(
                    {ActionType::Insert, Order(id, priceDist(rng_), qtyDist(rng_), side, OrderType::Limit)});
                ...
                actions.push_back({ActionType::Cancel, Order(idToCancel, 0, 0, Side::Buy, OrderType::Limit)});
            }
        }
        return actions;
    }
